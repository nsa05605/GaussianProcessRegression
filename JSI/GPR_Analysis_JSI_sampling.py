...
X_new = np.zeros((2751, 2))
Y_new = np.zeros((2751, 1))
...
output_file = "compare_Method1_Mat32_JSI_2Drad_GPR.txt"
'''
...
output_file = "compare_Method2_Mat_JSI_2Drad_GPR.txt"
'''
...
## 그냥 랜덤으로 샘플링
np.random.seed(0)
rand_arr = np.arange(0, 5503, 1)
result_arr = np.random.choice(rand_arr, 2751, replace=False)

for i in range(2751):
    X_new[i] = X[result_arr[i]]
    Y_new[i] = Y[result_arr[i]]

output_file = "compare_Method3_Mat32_JSI_2Drad_GPR.txt"
...
myAddMatExp = GPy.kern.MyAddMat32Exp(input_dim=2, variance=0.8041532729559738, lengthscale=3.4453764629757493)
myMulMatExp = GPy.kern.MyMulMat32Exp(input_dim=2, variance=0.772327416232277, lengthscale=6.788298370814227)

bias        = GPy.kern.Bias(input_dim=2, variance=0.33672953444342374)
...
m = GPy.core.GP(X=X_new, Y=Y_new, likelihood=poisson_likelihood, inference_method=laplace_inf, kernel=kernel)
